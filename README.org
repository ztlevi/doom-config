...
- [[#faq][FAQ]]
  ...
  - [[#when-you-get-too-many-open-files-issue-on-macos][When you get too many open files issue on MacOS]]
...
** When you get too many open files issue on MacOS
...
Run `M-x dap-cpptools-setup` to download the `vscode-cpptools` debug adapter.
